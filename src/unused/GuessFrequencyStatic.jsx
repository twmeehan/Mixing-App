...
function GuessNumberGame() {
  ...
  const updateEq = (number) => {
    ...
    const minFreq = 20;
    const maxFreq = 8000;
    const scaled = (number / 8000) * (maxFreq - minFreq) + minFreq;
  ...
  const toggleEq = async () => {
    ...
    try {
      await fetch(`${API_BASE}/toggle-eq?enabled=${next}`, { method: "GET" });

      // Hot-swap the stream without a “fresh start”
      // (cache-bust so the browser requests the new EQ state)
      const newSrc = STREAM_URL + `?v=${Date.now()}`;
      audioRef.current.src = newSrc;

      // Set position back; the browser will issue Range requests
